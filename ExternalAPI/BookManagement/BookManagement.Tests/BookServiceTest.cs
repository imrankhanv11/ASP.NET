    ...
    public class BookServiceTest
    {
        ...
        public async Task GetBookById_ShouldReturnBook_WhenBookExists()
        {
            ...
            using (new AssertionScope())
            {
                ...
                // Verify
                _mockGenericRepo.Verify(v => v.GetByIdAsync(It.IsAny<int>()), Times.Once());
                _mockMapper.Verify(v => v.Map<GetOneBookDTO>(It.IsAny<Book>()), Times.Once());
            }
        }
        ...
        public async Task GetAllBooks_ShouldReturnList_WhenBooksExist()
        {
            ...
            using( new AssertionScope())
            {
                ...
                result.Last().Title.Should().Be(fakeBooks.Last().Title);

                //Verify
                _mockProductRepo.Verify(v=> v.GetAllAsync(), Times.Once());
                _mockMapper.Verify(v => v.Map<IEnumerable<GetAllBooksDTO>>(It.IsAny<IEnumerable<Book>>()), Times.Once());
            }
        }
        ...
        public async Task AddBookService_ShouldReturnId_WhenBookAdded()
        {
            ...
            _mockGenericRepo.Setup(m=> m.AddAsync(It.IsAny<Book>())).ReturnsAsync(fakeproduct);
        ...
        public async Task DeleteBookService_ShouldDelete_WhenBookExists()
        {
            ...
            using(new AssertionScope())
            {
                ...
                // Verify
                _mockGenericRepo.Verify(r => r.DeleteAsync(It.IsAny<int>()), Times.Once());
                _mockGenericRepo.Verify(r => r.GetByIdAsync(It.IsAny<int>()), Times.Once());
            }
        }
        ...
        public async Task DeleteBookService_ShouldThrowException_WhenBookDoesNotExist()
        {
            ...
            using(new AssertionScope())
            {
                ...
                await act.Should().ThrowAsync<KeyNotFoundException>()
                    .WithMessage("Book with id 2 does not exist.");
        ...
        public async Task GetOneBokService_ShouldThrowError_WhenBookNotExists()
        {
            ...
            //--------------------------------------------------------------
            //// Act
            //Func<Task> act = () => await _service.GetOnebookService(book);
            ...
            //--------------------------------------------------------------------
            //await FluentActions.Invoking(() => _service.GetOnebookService(book))
            //    .Should()
